...
esp_err_t my_ulp_riscv_load_binary(const uint8_t* program_binary, size_t program_size_bytes)
{
    ...
    hal_memset(base, 0, CONFIG_ULP_COPROC_RESERVE_MEM); // 4KB
...
static void init_ulp_program(void)
{
    esp_err_t err = my_ulp_riscv_load_binary(ulp_main_bin_start, (ulp_main_bin_end - ulp_main_bin_start));
    ESP_ERROR_CHECK(err);
