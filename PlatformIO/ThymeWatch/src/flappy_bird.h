...
// eggfly mod
#define GAPHEIGHT        45     // pipe gap height

// floor size
// #define FLOORH           40     // floor height (from bottom of the screen)
// eggfly mod
#define FLOORH           18     // floor height (from bottom of the screen)
...
// background
const unsigned int BCKGRDCOL = THREE_BIT_BLUE;
// bird
const unsigned int BIRDCOL = THREE_BIT_YELLOW;
// pipe
const unsigned int PIPECOL = THREE_BIT_GREEN;
// pipe highlight
const unsigned int PIPEHIGHCOL  = THREE_BIT_WHITE;
// pipe seam
const unsigned int PIPESEAMCOL  = 0;
// floor
const unsigned int FLOORCOL = THREE_BIT_YELLOW;
// grass (col2 is the stripe color)
const unsigned int GRASSCOL  =  THREE_BIT_GREEN;
const unsigned int GRASSCOL2 =  THREE_BIT_CYAN;

// bird sprite
// bird sprite colors (Cx name for values to keep the array readable)
#define C0 BCKGRDCOL
#define C1 THREE_BIT_BLACK
#define C2 BIRDCOL
#define C3 WHITE
#define C4 RED
#define C5 THREE_BIT_MAGENTA
...
void game_loop() {
  ...
  while (1) {
    ...
    if (pipes.x <= TFTW) {
      ...
      // eggfly add:
      canvas.drawFastVLine(pipes.x + PIPEW - 1, 0, GAMEH, BCKGRDCOL);
    }
    // PIPECOL
#endif
    // bird
...
void game_start() {
  ...
  canvas.fillRect(0, TFTH2 - 10, TFTW, 1, WHITE);
  canvas.fillRect(0, TFTH2 + 15, TFTW, 1, WHITE);
  canvas.setTextColor(WHITE);
  ...
  canvas.println("-BIRD-");
  canvas.setTextSize(1);
  canvas.setCursor(35, TFTH2 - 21);
  canvas.println("ThymeWatch");
...
void game_over() {
  ...
  canvas.setTextColor(THREE_BIT_MAGENTA);
  if (score > maxScore) {
    EEPROM_Write(&score);
    maxScore = score;
    canvas.setTextSize(1);
