from machine import SPI, Pin, Timer
import framebuf,time

BitReverseTable256=[
        0X00,]

bblist=[]
for i in BitReverseTable256:
    bblist.append(i.to_bytes(1, 'little'))

class SharpMemLCD(framebuf.FrameBuffer):
    '''
    MicroPython driver to write to Sharp's Memory LCD. Should works with any standard pyboard
    Uses the built-in framebuffer class to format the buffer 
    Custom parameters (display pin, cs pin, screen's x and y dimensions, SPI channel) can be changed below
    '''
    CS = 8
    XDIM = 128
    YDIM = 128

    def __init__(self):
        # define pins
        self.spi = SPI(1, baudrate=1100000, polarity=0, phase=0, sck=Pin(6), mosi=Pin(7))
        self.cs = Pin(self.CS, Pin.OUT)
        self.set_cs(0)
        # define structures, related info
        self._xdim = self.XDIM
        self._ydim = self.YDIM
        self.buffer = bytearray((self._xdim//8) * self._ydim)
        super().__init__(self.buffer, self._xdim, self._ydim, framebuf.MONO_HLSB)
        self.vcom = 2
        # begin by clearing screen
        #self.clear()
    
    @property
    def xdim(self):
        return self._xdim

    @property
    def ydim(self):
        return self._ydim

    def toggle_vcom(self):
        self.vcom ^= 2

    # send lcd command to clear screen
    def clear(self):
        self.set_cs(1)
        time.sleep_us(6)
        sv=4 | self.vcom
        self.send(BitReverseTable256[sv])
        self.sendcmd(b'\x00')
        time.sleep_us(1)          
        self.set_cs(0)
        time.sleep_us(1)          
        self.toggle_vcom()

    def set_cs(self, logic_level):
        self.cs.value(logic_level)

    def send(self, value):
        self.spi.write(value.to_bytes(1, 'little'))
        
    def sendcmd(self,cmd):
        self.spi.write(cmd)

    def show(self):
        index = 0
        for line in range(self._ydim):
            self.set_cs(1)
            self.sendcmd(b'\x80')
            self.sendcmd(bblist[line+1])
            self.sendcmd(self.buffer[line*16:line*16+16])
            self.sendcmd(b'\x00')
            self.set_cs(0)      